...
const downloadManualUpdateInstaller = async () => {
  ...
  const asset = (release.assets || []).find((item: any) => /^RyuSAK-.*-Setup\.exe$/i.test(item.name || ""));

  if (!asset?.browser_download_url) {
    throw new Error("Could not find a RyuSAK setup installer asset in the latest GitHub release.");
  }
  ...
  if (!assetResponse.ok || !assetResponse.body) {
    throw new Error(`Installer download failed with ${assetResponse.status} ${assetResponse.statusText}`);
  }

  await pipeline(assetResponse.body as any, fs.createWriteStream(installerPath));
  return installerPath;
};

ipcMain.handle("install-manual-update", async () => {
  try {
    const installerPath = await downloadManualUpdateInstaller();
